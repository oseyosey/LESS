...
def get_tydiqa_dataset(data_dir: str,
    ...
    file = os.path.join(f"{data_dir}/eval/tydiqa/dev", file_name) #* Fix: path to tydiqa dataset is incorrect in original repo.
...
def get_tydiqa_raw_dataset(data_dir: str,                       
                           use_chat_format: bool = True,
                           chat_format: str = "tulu",
                           zh: bool = False):
    """
    ...
    for i, lang in enumerate(examples):
        ...
        dataset.update({lang: prompt + answer}) 
...
def get_raw_val_dataset(task_name, data_dir):
    if task_name == "bbh":
        ...
        return get_tydiqa_raw_dataset(data_dir)
    elif task_name == "mmlu":
